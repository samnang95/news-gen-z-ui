...
import { getCategories, getNews, getAllUsers, createNews, updateNews, deleteNews, updateUser, deleteUser, createCategory, updateCategory, deleteCategory, API_BASE_URL } from "@/services/api";
...
export default function Dashboard() {
  ...
  // Modal states for Category Operations
  ...
  const handleCategorySubmit = async (e: React.FormEvent) => {
    ...
    if (editingCategoryId) {
      await updateCategory(token, editingCategoryId, { name: categoryName });
    } else {
        ...
        <div className="p-8 max-w-6xl w-full mx-auto">
          ...
          {showCategoryModal && (
             <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center">
                <form onSubmit={handleCategorySubmit} className="bg-white p-8 rounded-2xl w-full max-w-md shadow-2xl relative">
                   <button type="button" onClick={() => { setShowCategoryModal(false); setEditingCategoryId(null); }} className="absolute top-4 right-4 text-zinc-400 hover:text-zinc-600">
                     <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                   </button>
                   <h2 className="text-2xl font-bold mb-6 text-zinc-800">{editingCategoryId ? "Edit Category" : "New Category"}</h2>
                   <div className="mb-8">
                     <label className="text-xs font-bold text-zinc-500 uppercase">Category Name</label>
                     <input type="text" required value={categoryName} onChange={e => setCategoryName(e.target.value)} className="w-full border-b border-zinc-200 py-2 focus:border-blue-500 focus:outline-none text-zinc-800" />
                   </div>
          ...
          {activeTab === 'Categories' && (
            <div className="bg-white rounded-1.5rem shadow-sm border border-zinc-100 overflow-hidden mt-8">
               <div className="p-6 border-b border-zinc-100 flex justify-between items-center bg-white">
                 ...
                 <button onClick={() => { setEditingCategoryId(null); setCategoryName(""); setShowCategoryModal(true); }} className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold px-5 py-2.5 rounded-xl shadow-[0_5px_15px_-5px_rgba(37,99,235,0.4)] transition-colors flex items-center">
                    <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4"></path></svg>
                    New Category
                 </button>
               </div>
                    ...
                    <tbody>
                      {filteredCategories.map((item: any) => (
                         <tr key={item._id} className="border-b border-zinc-50 hover:bg-zinc-50/50 transition-colors">
                             ...
                             <td className="px-6 py-5">{new Date(item.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</td>
                             <td className="px-6 py-5 text-center">
                                <button onClick={() => handleEditCategory(item)} className="text-blue-500 hover:text-blue-700 hover:bg-blue-50 p-2 rounded-full transition-colors inline-block mr-2" title="Edit">
                                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036L4 20v4h4L21 6.768a2 2 0 00-2.828-2.828z"></path></svg>
                                </button>
                                <button onClick={() => handleDeleteCategory(item._id)} className="text-red-400 hover:text-red-600 hover:bg-red-50 p-2 rounded-full transition-colors inline-block" title="Delete">
                                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                </button>
                             </td>
                          </tr>
                      ...
                      {filteredCategories.length === 0 && (
                         <tr><td colSpan={3} className="px-6 py-12 text-center text-zinc-400">No categories found.</td></tr>
                      )}
                    </tbody>
                  </table>
               </div>
